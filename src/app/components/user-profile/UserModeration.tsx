      ...
      <SettingTile>
        <Box direction="Column" gap="200">
          <Box gap="200" justifyContent="SpaceBetween">
            <Text size="L400">{t('kickedUser', { defaultValue: 'Kicked User' })}</Text>
            {time && date && (
              <Text size="T200">
                {date} {time}
              </Text>
          ...
          <Box direction="Column">
            {kickedBy && (
              <Text size="T200">
                {t('kickedBy', { defaultValue: 'Kicked by:' })} <b>{kickedBy}</b>
              </Text>
            )}
            <Text size="T200">
              {reason ? (
                <>
                  {t('reason', { defaultValue: 'Reason:' })} <b>{reason}</b>
                </>
              ) : (
                <i>{t('noReasonProvided', { defaultValue: 'No Reason Provided.' })}</i>
              )}
            </Text>
          </Box>
        </Box>
  ...
  return (
    <CutoutCard style={{ padding: config.space.S200 }} variant="Critical">
      <SettingTile>
        <Box direction="Column" gap="200">
          <Box gap="200" justifyContent="SpaceBetween">
            <Text size="L400">{t('bannedUser', { defaultValue: 'Banned User' })}</Text>
            {time && date && (
              <Text size="T200">
                {date} {time}
              </Text>
          ...
          <Box direction="Column">
            {bannedBy && (
              <Text size="T200">
                {t('bannedBy', { defaultValue: 'Banned by:' })} <b>{bannedBy}</b>
              </Text>
            )}
            <Text size="T200">
              {reason ? (
          ...
          {canUnban && (
            <Button
              ...
              disabled={banning}
            >
              <Text size="B300">{t('unban', { defaultValue: 'Unban' })}</Text>
            </Button>
          )}
        </Box>
      </SettingTile>
  ...
  return (
    <CutoutCard style={{ padding: config.space.S200 }} variant="Success">
      <SettingTile>
        <Box direction="Column" gap="200">
          <Box gap="200" justifyContent="SpaceBetween">
            <Text size="L400">{t('invitedUser', { defaultValue: 'Invited User' })}</Text>
            {time && date && (
              <Text size="T200">
                {date} {time}
              </Text>
          ...
          <Box direction="Column">
            {invitedBy && (
              <Text size="T200">
                {t('invitedBy', { defaultValue: 'Invited by:' })} <b>{invitedBy}</b>
              </Text>
            )}
            <Text size="T200">
              {reason ? (
          ...
          {canKick && (
            <Button
              ...
              disabled={kicking}
            >
              <Text size="B300">{t('cancelInvite', { defaultValue: 'Cancel Invite' })}</Text>
            </Button>
          )}
        </Box>
      </SettingTile>
    ...
    <Box direction="Column" gap="400">
      <Box direction="Column" gap="200">
        <Box grow="Yes" direction="Column" gap="100">
          <Text size="L400">{t('moderation', { defaultValue: 'Moderation' })}</Text>
          <Input
            ref={reasonInputRef}
            placeholder={t('reasonPlaceholder', { defaultValue: 'Reason' })}
            size="300"
            variant="Background"
            radii="300"
            disabled={disabled}
        ...
        <Box shrink="No" gap="200">
          {canInvite && (
            <Button
              ...
              disabled={disabled}
            >
              <Text size="B300">{t('invite', { defaultValue: 'Invite' })}</Text>
            </Button>
          )}
          {canKick && (
            <Button
              ...
              disabled={disabled}
            >
              <Text size="B300">{t('kick', { defaultValue: 'Kick' })}</Text>
            </Button>
          )}
          {canBan && (
            <Button
              ...
              disabled={disabled}
            >
              <Text size="B300">{t('ban', { defaultValue: 'Ban' })}</Text>
            </Button>
          )}
        </Box>
      </Box>
