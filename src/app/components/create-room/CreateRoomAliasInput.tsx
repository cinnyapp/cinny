...
export function CreateRoomAliasInput({ disabled }: { disabled?: boolean }) {
  ...
  return (
    <Box shrink="No" direction="Column" gap="100">
      <Text size="L400">{t('addressOptional', { defaultValue: 'Address (Optional)' })}</Text>
      <Text size="T200" priority="300">
        {t('addressOptionalDescription', {
          defaultValue: 'Pick an unique address to make it discoverable.',
        })}
      </Text>
      <Input
        ref={aliasInputRef}
      ...
      {aliasAvailable === false && (
        <Box style={{ color: color.Critical.Main }} alignItems="Center" gap="100">
          ...
          <Text size="T200">
            <b>
              {t('addressTaken', {
                defaultValue: 'This address is already taken. Please select a different one.',
              })}
            </b>
          </Text>
        </Box>
