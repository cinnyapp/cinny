  ...
  return (
    <RoomInputPlaceholder alignItems="Center" gap="600" className={css.RoomTombstone}>
      <Box direction="Column" grow="Yes">
        <Text size="T400">
          {body ||
            t('roomReplacedFallback', {
              defaultValue: 'This room has been replaced and is no longer active.',
            })}
        </Text>
        {joinState.status === AsyncStatus.Error && (
          <Text style={{ color: color.Critical.Main }} size="T200">
            {(joinState.error as any)?.message ??
              t('failedToJoinReplacementRoom', {
                defaultValue: 'Failed to join replacement room!',
              })}
          </Text>
        )}
      </Box>
      <Box shrink="No">
        ...
        joinState.status === AsyncStatus.Success ? (
          <Button onClick={handleOpen} size="300" variant="Success" fill="Solid" radii="300">
            <Text size="B300">{t('openNewRoom', { defaultValue: 'Open New Room' })}</Text>
          </Button>
        ) : (
          <Button
            onClick={handleJoin}
          ...
          >
            <Text size="B300">{t('joinNewRoom', { defaultValue: 'Join New Room' })}</Text>
          </Button>
        )}
      </Box>
