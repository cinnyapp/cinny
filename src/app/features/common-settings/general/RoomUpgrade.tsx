          ...
          <Dialog variant="Surface">
            ...
            >
              <Box grow="Yes">
                <Text size="H4">
                  {room.isSpaceRoom()
                    ? t('spaceUpgradeTitle', { defaultValue: 'Space Upgrade' })
                    : t('roomUpgradeTitle', { defaultValue: 'Room Upgrade' })}
                </Text>
              </Box>
            ...
            <Box style={{ padding: config.space.S400 }} direction="Column" gap="400">
              <Text priority="400" style={{ color: color.Critical.Main }}>
                <b>{t('irreversibleAction', { defaultValue: 'This action is irreversible!' })}</b>
              </Text>
              ...
              >
                <Text size="B400">
                  {room.isSpaceRoom()
                    ? t('upgradeSpace', { defaultValue: 'Upgrade Space' })
                    : t('upgradeRoom', { defaultValue: 'Upgrade Room' })}
                </Text>
              </Button>
            </Box>
          </Dialog>
        ...
        description={
          replacementRoom
            ? tombstoneContent.body ||
              (room.isSpaceRoom()
                ? t('replacedSpace', { defaultValue: 'This space has been replaced!' })
                : t('replacedRoom', { defaultValue: 'This room has been replaced!' }))
            : t('currentVersion', { defaultValue: 'Current version: {{version}}.', version: roomVersion })
        }
        after={
          <Box alignItems="Center" gap="200">
            {predecessorRoomId && (
              ...
              >
                <Text size="B300">
                  {room.isSpaceRoom()
                    ? t('oldSpace', { defaultValue: 'Old Space' })
                    : t('oldRoom', { defaultValue: 'Old Room' })}
                </Text>
              </Button>
            )}
            {replacementRoom ? (
              ...
              >
                <Text size="B300">
                  {room.isSpaceRoom()
                    ? t('openNewSpace', { defaultValue: 'Open New Space' })
                    : t('openNewRoom', { defaultValue: 'Open New Room' })}
                </Text>
              </Button>
            ) : (
              <Button
              ...
              >
                <Text size="B300">{t('upgrade', { defaultValue: 'Upgrade' })}</Text>
              </Button>
            )}
          </Box>
        }
