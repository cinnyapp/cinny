  ...
  return (
    <Box as="form" direction="Column" gap="100" onSubmit={handleSearchSubmit}>
      ...
      <Input
        ...
        size="500"
        variant="Background"
        placeholder={t('searchForKeyword', { defaultValue: 'Search for keyword' })}
        autoComplete="off"
        before={
          active && loading ? (
        ...
        after={
          active ? (
            <Chip
              ...
              onClick={onReset}
            >
              <Text size="B300">{t('clear', { defaultValue: 'Clear' })}</Text>
            </Chip>
          ) : (
            <Chip type="submit" variant="Primary" size="400" radii="Pill" outlined>
              <Text size="B300">{t('enter', { defaultValue: 'Enter' })}</Text>
            </Chip>
          )
        }
      />
