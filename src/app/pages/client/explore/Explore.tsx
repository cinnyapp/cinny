    ...
    <>
        ...
        <OverlayCenter>
          ...
          >
            <Dialog variant="Surface">
              ...
              >
                <Box grow="Yes">
                  <Text size="H4">{t('addServerTitle', { defaultValue: 'Add Server' })}</Text>
                </Box>
              ...
              >
                <Text priority="400">
                  {t('addServerDescription', {
                    defaultValue: 'Add server name to explore public communities.',
                  })}
                </Text>
                <Box direction="Column" gap="100">
                  <Text size="L400">{t('serverNameLabel', { defaultValue: 'Server Name' })}</Text>
                  <Input ref={serverInputRef} name="serverInput" variant="Background" required />
                  {exploreState.status === AsyncStatus.Error && (
                    <Text style={{ color: color.Critical.Main }} size="T300">
                      {t('loadPublicRoomsFailed', {
                        defaultValue: 'Failed to load public rooms. Please try again.',
                      })}
                    </Text>
                  )}
                </Box>
                <Box direction="Column" gap="200">
                  <Button type="submit" onClick={handleView} variant="Secondary" fill="Soft">
                    <Text size="B400">{t('viewButton', { defaultValue: 'View' })}</Text>
                  </Button>
                </Box>
              </Box>
            </Dialog>
      ...
      >
        <Text size="B300" truncate>
          {t('addServerTitle', { defaultValue: 'Add Server' })}
        </Text>
      </Button>
    </>
  );
        ...
        <Box grow="Yes" gap="300">
          <Box grow="Yes">
            <Text size="H4" truncate>
              {t('exploreCommunityTitle', { defaultValue: 'Explore Community' })}
            </Text>
          </Box>
        </Box>
      </PageNavHeader>
                  ...
                  <Box as="span" grow="Yes" alignItems="Center" gap="200">
                    ...
                    <Box as="span" grow="Yes">
                      <Text as="span" size="Inherit" truncate>
                        {t('featuredLabel', { defaultValue: 'Featured' })}
                      </Text>
                    </Box>
                  </Box>
                </NavItemContent>
            ...
            <NavCategory>
              <NavCategoryHeader>
                <Text size="O400" style={{ paddingLeft: config.space.S200 }}>
                  {t('serversHeader', { defaultValue: 'Servers' })}
                </Text>
              </NavCategoryHeader>
              {servers.map((server) => (
                <NavItem
